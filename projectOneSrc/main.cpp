...
{
    infile.open("./demonstration2.txt");
}
...
index = 0;
    ...
    if(pid == 0)
    {
        ...
        for(int indec = 0; indec < 50; indec++)
        {
            cout<<"\n interrupt instrunction["<<indec<<"] = "<<memory_obj.list[indec];
        }

//
//       for(int i = 1000; i < 1071 ; i++)
//       {
//           cout<<"\n interrupt instrunction["<<i<<"] = "<<memory_obj.list[i];
//       }
//
//       for(int j = 1500; j < 1570; j++)
//       {
//           cout<<"\n int instruction["<<j<<"]"<<memory_obj.list[j];
//       }
//
//       for(int k = 1700 ; k < 1770; k++)
//       {
//           cout<<"\nint instruction at 1700s["<<k<<"]"<<memory_obj.list[k];
//       }
    }
    ...
    infile.close();
    index = 0;
    while ((processor_obj.IR != 50  && index < 100)  ) {
...
//                        if((timer % 10 == 0) && (kernal_mode == 0))
//                        {
//                            // make IR a sentinel
//                            // so that the interupt instructions can be performed
//                            processor_obj.IR = -1000;
//                        }
                    ...
                    switch (processor_obj.IR) {
                        case 0:
                            ...
                            read(pipefds2[0], &processor_obj.IR,
                                 sizeof(int)); // read instruction from memory where PC points
                        ...
                        case 6:
                            ...
                            processor_obj.SP[0] =
                                    processor_obj.SP[0] + processor_obj.X;  // change the SP pointer to X + SP
                            ...
                            write(pipefds1[1], &processor_obj.PC, sizeof(int));
                            read(pipefds2[0], &processor_obj.IR, sizeof(int));
                                cout<<"\nSix case"<<"IR = "<<processor_obj.IR <<"PC = "<<processor_obj.PC <<"X = "<<processor_obj.X <<"AC ="<< processor_obj.AC << "Y = "<< processor_obj.Y << "SP "<<processor_obj.SP[0] << ":"<<processor_obj.SP[1];
                            ++timer;
                            break;
                        case 7:
                            ...
                            write(pipefds1[1], &processor_obj.PC, sizeof(int));
                            read(pipefds2[0], &processor_obj.SP[0],
                                 sizeof(int)); // store that address in to sp data holding element

                            processor_obj.SP[1] = processor_obj.AC;

                            write(pipefds1[1], &processor_obj.SP,
                                  sizeof(processor_obj.SP)); // write AC into the address
                        ...
                        case 20:
                            ...
                            write(pipefds1[1], &processor_obj.PC, sizeof(int));
                            read(pipefds2[0], &processor_obj.IR,
                                 sizeof(int)); // get the instruction from that new address
                        ...
                        case 23:
                            ...
                            write(pipefds1[1], &processor_obj.SP,
                                  sizeof(processor_obj.SP)); // send the SP and the PC  to store PC into memory

                            read(pipefds2[0], &processor_obj.PC, sizeof(int));// PC jumps to new address

                            write(pipefds1[1], &processor_obj.PC,
                                  sizeof(int)); // grab new instruction from jumped address
                            read(pipefds2[0], &processor_obj.IR, sizeof(int));
                        ...
                        case 27:
                            ...
                            --processor_obj.SP[0]; // move the stack back one
                            write(pipefds1[1], &processor_obj.SP,
                                  sizeof(processor_obj.SP)); // send the SP and the AC to store PC into user stack memory
                            read(pipefds2[0], &processor_obj.SP[1],
                                 sizeof(int)); // receiving  the memmories write back just to unload it
                        ...
                        case 30:
                            ...
                            write(pipefds1[0], &processor_obj.SP[1], sizeof(int));
                            read(pipefds2[0], &processor_obj.SP[0], sizeof(int)); // restore the SP pointer

                            write(pipefds1[0], &processor_obj.PC, sizeof(int)); // go on to the next instruction
                        ...
                        default:
                            if(processor_obj.IR == -1000)
                            {
                                processor_obj.SP[1] = processor_obj.SP[0];// put the stack pointer in the element to be sent
                                processor_obj.SP[0] = 1999;

                                write(pipefds2[1], &processor_obj.SP, sizeof(processor_obj.SP)); // write the pc counter into the system stack
                                read(pipefds2[0],&processor_obj.SP[1],sizeof(int));

                                processor_obj.SP[0] = 1998;
                                processor_obj.SP[1] = processor_obj.PC;
                                write(pipefds2[1], &processor_obj.SP, sizeof(processor_obj.SP));
                            ...
                            else
                            {
                                ...
                                processor_obj.SP[0] = 1998;
                                processor_obj.SP[1] = processor_obj.PC;
                                write(pipefds2[1], &processor_obj.SP, sizeof(processor_obj.SP)); // write the
                            ...
                            cout << "\ndefault case" << "IR = " << processor_obj.IR << "PC = " << processor_obj.PC
                                 << "X = " << processor_obj.X << "AC =" << processor_obj.AC<< " SP "<< processor_obj.SP[0]<<":" << processor_obj.SP[1];
